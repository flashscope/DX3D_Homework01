...
D3DMATERIAL9*       g_pMeshMaterials1 = NULL; // Materials for our mesh
LPDIRECT3DTEXTURE9* g_pMeshTextures1 = NULL; // Textures for our mesh
...
void DXTutorial::Create( int width, int height )
{
...
HRESULT InitGeometry()
{
	...
	if( FAILED( D3DXLoadMeshFromX( L"tiger.x", D3DXMESH_SYSTEMMEM,
		g_pd3dDevice, NULL,
		&pD3DXMtrlBuffer1, NULL, &g_dwNumMaterials1,
		&g_pMesh1 ) ) )
	{
	...
	if ( !(g_pMesh1->GetFVF()&D3DFVF_NORMAL) )
	{
		ID3DXMesh* pTempMesh = 0;
		g_pMesh1->CloneMeshFVF(D3DXMESH_MANAGED, g_pMesh1->GetFVF() | D3DFVF_NORMAL, g_pd3dDevice, &pTempMesh);
	...
	// pD3DXMtrlBuffer
	D3DXMATERIAL* d3dxMaterials = ( D3DXMATERIAL* )pD3DXMtrlBuffer1->GetBufferPointer();
	g_pMeshMaterials1 = new D3DMATERIAL9[g_dwNumMaterials1];
	if( g_pMeshMaterials1 == NULL )
		return E_OUTOFMEMORY;
	g_pMeshTextures1 = new LPDIRECT3DTEXTURE9[g_dwNumMaterials1];
	if( g_pMeshTextures1 == NULL )
		return E_OUTOFMEMORY;
...
VOID SetupLights()
{
	...
	g_pd3dDevice->SetRenderState( D3DRS_AMBIENT, 0x00999999 );
}
...
VOID Render()
{
	...
	if( SUCCEEDED( g_pd3dDevice->BeginScene() ) )
	{
		...
		D3DXMATRIXA16 matWorld;
		//D3DXMatrixIdentity( &matWorld );
		D3DXMatrixTranslation(&matWorld, 2, 0, 0);
